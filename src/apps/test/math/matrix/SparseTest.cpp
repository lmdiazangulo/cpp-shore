...
#include "math/matrix/Sparse.h"

using namespace SEMBA;
using namespace Math;
using namespace Matrix;

class MathMatrixSparseTest : public ::testing::Test {

protected:
   virtual void SetUp() {
      // Inits. a matrix.
      a_ = Sparse<double>(2,2);
      a_(0,0) = 1.0; a_(0,1) = 2.0;
      a_(1,0) = 3.0; a_(1,1) = 5.0;
      // Inits. b_ matrix.
      b_ = Sparse<double>(2,2);
...
TEST_F(MathMatrixSparseTest, BasicOperations) {
   Sparse<double> c(2,2);
   // Addition
...
TEST_F(MathMatrixSparseTest, Inversion) {
   // 2x2
   Sparse<double> c(2,2), aux(2,2);
   c(0,0) = -5.0; c(0,1) =  2.0;
   c(1,0) =  3.0; c(1,1) = -1.0;
